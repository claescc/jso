...
function leesVierkant(hoofdTabel){ // lees in alle getallen 
    if (isVierkant(hoofdTabel)){ // check ineens of het vriekant is -- dan ga door 
          for (var i = 0; i < hoofdTabel.length; i++){  // alle rijen 
...
function isVierkant(hoofdTabel) {           // is het een vriekant ?
    for (var i = 0; i < hoofdTabel.length; i++) {
        if (hoofdTabel.length != hoofdTabel[i].length) { // aantal rijen != aantal kollomen rij i 
            return false; // geen vrierkant  retrun = break 
        }
    }
    return true; // all rijen evenveel kollomen = true = vierkant
}

function somRij(hoofdTabel, rijNr) { // geef tabel en rij nummer blijft zelfde 
    var som = 0;
    for (var i = 0; i < hoofdTabel[rijNr].length; i++) { 
        som += parseInt(hoofdTabel[rijNr][i]);      // bereken SOM van 1 rij !!
    }
    return som;
}
...
function somDiagonaalRechtsNaarLinks(hoofdTabel) {      // rij en kollom = elkaar 
    var som = 0;
    for (var i = 0; i < hoofdTabel.length; i++) { // rijen afgaan
